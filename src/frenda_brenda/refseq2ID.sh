...
echo "Label,Type,StartingConc,EC,Conc,Mechanisms,Parameters" > "$output_file2"
...
for index in "${!values1[@]}"; do
    ...
    ec_number=$(echo "$conversion" | awk -F'\t' '{print $1}')

    awk -F'\t' -v OFS=',' '{ print $1, $2 }' <<< "$conversion" >> "$output_file1"

    echo ",Enzyme,$value2,$ec_number" >> "$output_file2"
done
